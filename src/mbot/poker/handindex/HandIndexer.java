...
import mbot.poker.handindex.HandIndexerState;
...
public class HandIndexer {
	...
	static {
		for (int i=0; i < 1<<(SUITS-1); ++i)
			for (int j=1; j < SUITS; j++)
				equal[i][j] = (i&1<<(j-1)) == 0 ? false : true;

		for (int i=0; i < 1<<RANKS; ++i)
			for (int j=0, set=~i&(1<<RANKS)-1; j < RANKS; ++j, set&=set-1)
				nthUnset[i][j] = set == 0 ? 0xff : Integer.numberOfTrailingZeros(set);
		...
		for (int i = 0; i < 1 << RANKS; ++i) {
			for (int set = i, j = 1; set != 0; ++j, set &= set - 1) {
				rankSetToIndex[i] += nCrRanks[Integer.numberOfTrailingZeros(set)][j];
			}
		...
		suitPermutations = new int[numPermutations][SUITS];
		
		for (int i=0; i<numPermutations; ++i) {
			for (int j=0, index=i, used=0; j<SUITS; ++j) {
				int suit = index%(SUITS-j); index /= SUITS-j;
			...
			if (i+1 < SUITS && equal[equalIndex][i+1]) {
				if (i+2 < SUITS && equal[equalIndex][i+2]) {
					if (i+3 < SUITS && equal[equalIndex][i+3]) {
						swap(suitIndex, i, i+1); swap(suitIndex, i+2, i+3); swap(suitIndex, i, i+2); swap(suitIndex, i+1, i+3); swap(suitIndex, i+1, i+2);
						part = suitIndex[i] + nCrGroups[suitIndex[i+1]+1][2] + nCrGroups[suitIndex[i+2]+2][3] + nCrGroups[suitIndex[i+3]+3][4];
						size = nCrGroups[suitMultiplier[i]+3][4];
						i += 4;
					} else {
						swap(suitIndex, i, i+1); swap(suitIndex, i, i+2); swap(suitIndex, i+1, i+2);
						part = suitIndex[i] + nCrGroups[suitIndex[i+1]+1][2] + nCrGroups[suitIndex[i+2]+2][3];
	...
	public boolean unindex(int round, long index, int[] cards) {
		...
		for (int i=0; i<SUITS;) {
			int j=i+1;
			for(; j<SUITS && configuration[round][configurationIdx][j] == configuration[round][configurationIdx][i]; ++j) {}

			int suitSize  = configurationToSuitSize[round][configurationIdx][i];
			long groupSize  = nCrGroups[suitSize+j-i-1][j-i];
			long groupIndex = Long.remainderUnsigned(index, groupSize); index = Long.divideUnsigned(index, groupSize);

			for(; i<j-1; ++i) {
				suitIndex[i] = low = (int) Math.floor(Math.exp(Math.log(groupIndex)/(j-i) - 1 + Math.log(j-i))-j-i);
				...
				//for(suitIndex[i]=0; nCrGroups[(int) (suitIndex[i]+1+j-i-1)][j-i] <= groupIndex; ++suitIndex[i]) {}
				groupIndex -= nCrGroups[(int) (suitIndex[i]+j-i-1)][j-i]; 
			}

			suitIndex[i] = groupIndex; ++i;
		}

		int[] location = new int[rounds]; System.arraycopy(roundStart, 0, location, 0, rounds);
		for(int i=0; i<SUITS; ++i) {
			int used = 0, m = 0;
			for(int j=0; j<rounds; ++j) {
				int n              = configuration[round][configurationIdx][i]>>ROUND_SHIFT*(rounds-j-1)&ROUND_MASK;
				int roundSize     = nCrRanks[RANKS-m][n]; m += n;
				int roundIdx      = (int) Long.remainderUnsigned(suitIndex[i],roundSize); suitIndex[i] = Long.divideUnsigned(suitIndex[i], roundSize);
				int shiftedCards  = indexToRankSet[n][roundIdx], rankSet = 0;
				for(int k=0; k<n; ++k) {
					int shiftedCard = shiftedCards&-shiftedCards; shiftedCards ^= shiftedCard;
					int card         = nthUnset[used][Integer.numberOfTrailingZeros(shiftedCard)]; rankSet |= 1<<card;
	...
	private final void swap(final int[] suitIndex, final int u, final int v) {
	...
	private void enumerateConfigurationsR(int round, int remaining, int suit, int equal, int[] used, int[] configuration, boolean tabulate) {
		if (suit == SUITS) {
			...
			for (int i=min; i<=max; ++i) {
				...
				enumerateConfigurationsR(round, remaining-i, suit+1, newEqual, used, configuration, tabulate);
